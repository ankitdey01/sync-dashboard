...
function positionOnRing(ring: OrbitRing, angle: number): CSSProperties {
  ...
  const r = RADIUS[ring];
  // Integers only: identical serialization on server and client (no hydration mismatch).
  return {
    left: Math.round(CENTER.x + r * Math.cos(rad)),
    top: Math.round(CENTER.y - r * Math.sin(rad)),
  };
}
...
function OrbitStatus({ label }: OrbitStatusItem) {
  return (
    <div className="flex h-[30px] items-center gap-1.5 whitespace-nowrap rounded-full border border-[#e5e7eb] bg-[#f5f5f5] px-2.5 text-[13.5px] font-medium text-[#111111] shadow-[0_2px_6px_rgba(0,0,0,0.05)] dark:border-white/10 dark:bg-[#1a1a1a] dark:text-white">
      <CircleCheck size={15} strokeWidth={2.2} className="fill-[#111111] text-[#f5f5f5] dark:fill-[#ffffff] dark:text-[#1a1a1a]" />
      {label}
    </div>
  );
}
...
function OrbitCheck() {
  return (
    <div className="flex h-[50px] w-[50px] items-center justify-center rounded-full border border-[#111111] bg-[#111111] shadow-[0_2px_8px_rgba(0,0,0,0.06)] dark:border-white dark:bg-white">
      <CircleCheck size={17} strokeWidth={2.4} className="fill-white text-[#111111] dark:fill-[#0a0a0a] dark:text-white" />
    </div>
  );
}
...
export default function CommunityOrbit({
  ...
  // Render floating items client-side only: motion style serialization
  // differs between SSR and hydration, which triggers hydration mismatches.
  // Server snapshot (false) matches SSR HTML; client snapshot (true) hydrates to mounted UI.
  ...
  return (
    <section
      className={`w-full bg-transparent px-4 pb-4 text-[#1f1f1f] dark:text-white ${className ?? ''}`}
    >
      <div
        ref={frameRef}
        ...
        >
          ...
          <div className="absolute left-1/2 top-[393px] grid -translate-x-1/2 auto-cols-fr grid-flow-col gap-7">
            {stats.map((s, i) => (
              ...
              >
                <span className="text-[48px] font-medium leading-none tracking-[-0.02em] text-[#111111] tabular-nums dark:text-white">
                  <CountUp value={s.value} delay={0.9 + i * 0.12} />
                </span>
                <span className="mt-[15px] text-[14px] leading-none text-[#6b7280] dark:text-white/50">
                  {s.label}
                </span>
              </motion.div>
            ))}
          ...
          {tags.map((t, i) => {
            ...
            return (
              <Tag
                ...
                whileHover={{ y: -2 }}
                whileTap={{ y: 0 }}
                className="group flex h-10 items-center gap-2.5 rounded-full border border-black/[0.08] bg-white pl-1.5 pr-4 text-[14px] font-medium text-[#3a3a3a] shadow-[0_1px_2px_rgba(0,0,0,0.04),inset_0_1px_0_rgba(255,255,255,0.8)] transition-all duration-200 hover:border-black/[0.14] hover:shadow-[0_6px_16px_-6px_rgba(0,0,0,0.18)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black/10 active:shadow-none dark:border-white/10 dark:bg-[#161616] dark:text-white/80 dark:shadow-none dark:focus-visible:ring-white/20 dark:hover:border-white/20 dark:hover:bg-[#1e1e1e]"
              >
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#f5f5f5] text-[#111111] transition-colors group-hover:bg-[#111111] group-hover:text-white dark:bg-[#1a1a1a] dark:text-white dark:group-hover:bg-white dark:group-hover:text-[#0a0a0a] [&>svg]:h-[15px] [&>svg]:w-[15px]">
                  {t.icon}
                </span>
                {t.label}
              </Tag>
